# -*- coding: utf-8 -*-
# @FileName: save_gt_mask_multiprocess.py
# @Time    : 22/9/25 20:28
# @Author  : Haiyang Mei
...
def save_gt_mask(path):
    ...
    save_root = '/workspace/RobotSeg/dataset/VRS/test/mask_gt_info'  # VRS dataset
    # save_root = '/workspace/RobotSeg/dataset/RoboEngine/test/mask_info'  # RoboEngine dataset
    ...
    for obj_dir in obj_dirs:
        obj_name = os.path.basename(obj_dir)  # '000', '001', '002'
        obj_id = int(obj_name)

        pngs = natsorted(glob(os.path.join(obj_dir, '*.png')))
        for idx_in_seq, png_path in enumerate(pngs):
            mask = cv2.imread(png_path)

            if mask is None:
                continue

            if idx_in_seq not in gt_state:
                gt_state[idx_in_seq] = {}
            ...
            for k, v in instance_mask.items():
                gt_state[idx_in_seq][k] = v
                instance_to_obj[k] = obj_id
    ...
    for instance_id in range(len(instance_color)):
        ...
        real_obj_id = instance_to_obj[instance_id]
        np.save(os.path.join(save_path, f'{real_obj_id:03d}.npy'), cur_mask)

def main():
    dataset_path = '/workspace/RobotSeg/dataset/VRS/test/mask_gt'  # VRS dataset
    # dataset_path = '/workspace/RobotSeg/dataset/RoboEngine/test/mask'  # RoboEngine dataset
